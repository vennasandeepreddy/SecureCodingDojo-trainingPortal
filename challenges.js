...
exports.getChallengeDefinitionsForUser = async (user, moduleId) => {
    ...
    if(moduleId=="blackBelt"){
        try{
            moduleDefinitions =  fs.readFileSync("./static/lessons/blackBelt/definitions.json",'utf-8')
            moduleDefinitions = JSON.parse(moduleDefinitions)
        }
        catch(err){
            moduleDefinitions=[]
        }
    ...
    let hideSolutionList={}
    
    try{
        hideSolutionList=fs.readFileSync("./configuration/viewSolutionButton.json",'utf-8')
        hideSolutionList=JSON.parse(hideSolutionList)
    }
    catch(err){
        hideSolutionList={}
    }
    ...
    for(let level of moduleDefinitions){
        if (permittedLevel >= level.level) {
            for(let challenge of level.challenges) {
                challenge.hideSolution=hideSolutionList && hideSolutionList[challenge.id]?true:false;
